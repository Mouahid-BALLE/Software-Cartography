...
""" 
    Description : A partir du fichier JSON généré par la recherche sur Hal ou SH, ce script extrait les URLs des dépôts GitHub 
    et récupère les informations détaillées de ces dépôts. Il utilise l'API GitHub pour collecter des métadonnées telles que 
    le nombre d'étoiles, de forks, les contributeurs, les langues de programmation utilisées, etc. 
    Les informations récupérées sont ensuite sauvegardées dans un fichier JSON de sortie.

    Entrées :
    - `input_filepath` : Chemin vers le fichier JSON d'entrée contenant les projets issus de Hal ou SH, avec des URLs de dépôts GitHub.
                         Ces fichiers JSON sont générés à partir des scripts de recherche Hal ou SH respectivement dans les dossier Hal et SH.

    Sorties :
    - `output_filepath` : Fichier JSON de sortie où seront sauvegardées les informations des dépôts GitHub.
"""
...
class GitHubRepoInfoCollector:
    ...
    def process_projects(self, projects):
        ...
        """
        results = {"projects": []}
        project_counter = 1  # Initialisation du compteur
        ...
        for project in projects:
            ...
            # Vérification de la validité de l'URL GitHub
            if "github.com" not in github_url:
                results["projects"].append({
                    "project_number": project_counter,
                    "title": project["title"],
...
def main():
    ...
    with open('../SH/SH_CNRS.json', 'r', encoding='utf-8') as f:
        projects_data = json.load(f)

    # Créer une instance de GitHubRepoInfoCollector avec votre jeton GitHub personnel
    github_collector = GitHubRepoInfoCollector(token="use_your_token_here")
    ...
    github_collector.save_json(results, 'CNRS_GITHUB_FROM_SH.json')
